...
from utils.misc import send_data, process_isolated
...
if __name__ == '__main__':
    ...
    data, _, selected_cells, mean, std, _, _ = send_data(args)
    total_communication_central += sizeof_data(data)

    test_data = data.iloc[-7:]
    train_data = data.iloc[:-7]

    # 학습 데이터를 하나의 Series로 합칩니다.
    combined_train_data = pd.concat([train_data[col] for col in train_data.columns])
    print(combined_train_data)

    # 테스트 데이터를 하나의 Series로 합칩니다.
    combined_test_data = pd.concat([test_data[col] for col in test_data.columns])
    print(combined_test_data)
    '''
    all_data = []
    for cell, dataloader in train.items():
        for data in dataloader:
            all_data.append(data)

    # 이제 all_data를 사용하여 하나의 DataLoader를 생성합니다.
    from torch.utils.data import DataLoader, ConcatDataset

    # Assuming each data in all_data is a TensorDataset or similar
    combined_dataset = ConcatDataset(all_data)
    combined_dataloader = DataLoader(combined_dataset, batch_size=args.local_bs, shuffle=True)
    '''
    ...
    # print(global_model)
    '''
    global_weights = central_model.state_dict()
    ...
    for epoch in tqdm.tqdm(range(args.epochs)):
        ...
        for cell in cell_idx:
            ...
            central_model.load_state_dict(global_weights)
            central_model.train()
